...
## Появились новые файлы — что делать

Автоматически ничего не подхватывается: бэкап — это снимок на момент запуска.
Но повторный запуск дешёвый, потому что обе половины пропускают неизменное.

Меню → **«Обновить и залить»**: пересканирует те профили, что уже есть в бэкапе,
добавит только новое и изменившееся, затем догрузит в R2 только новые объекты.

Как определяется «не изменился»: совпадают **размер и время изменения**, а блоб
на месте — тогда файл даже не читается. Поэтому повторный проход по 300 тысячам
файлов занимает минуты, а не часы.

Чего это не делает:
- **удалённые файлы остаются** в бэкапе (это защита, а не баг: удалил случайно —
  восстановишь). Убрать устаревшее: `prune`;
- файл, изменённый без изменения размера И времени, замечен не будет — редкий
  случай, обычно результат работы специального софта.

Перед переустановкой запусти «Обновить и залить» последний раз — чтобы в облаке
оказалась работа последних часов.

## Прерывание

Долгие операции прерываются: **Esc** или **Ctrl+C**.

- **Сканирование** — прерывается сразу, ничего не записано.
- **Упаковка** — файлы «в полёте» дописываются, манифест остаётся целым.
  Следующий запуск в ту же папку продолжит с этого места: файлы с тем же
  размером и временем изменения не читаются повторно.
- **Заливка/скачивание R2** — уже загруженные объекты не отправляются заново.

## Закрой приложения перед бэкапом

Запущенное приложение держит свои файлы, и копирование может **встать намертво** —
без ошибки, без загрузки процессора, без обращений к диску. Так себя повёл Chrome:
бэкап замер на файлах расширений и не двигался.

Перед бэкапом TUI сам проверяет запущенные приложения и предлагает закрыть их
(с кнопкой «Проверить снова»). Что желательно закрыть: **Chrome и другие браузеры**,
**Telegram**, **Discord**, **Element**, **KeePassXC**, **Obsidian**, кошельки, **OBS**.

Защита на случай, если что-то всё же держит файл: чтение прерывается по таймауту
в 90 секунд, файл помечается как заблокированный и пропускается, а бэкап идёт
дальше. Закрыть приложение и запустить бэкап снова в ту же папку — доберутся
